...
#### GUI INTERFACE v1 WITH STRICT PHOTO CONTROL AND MANUAL INPUT AND LOG_DTR
...
def main(stack_name,Width_dtr,Length_dtr):
    ...
    screenshot_count = 0
    L_text="NOT-OK"  ### INITIALIZE VARIABLE
    W_text = "NOT-OK" ### INITIALIZE VARIABLE
    ...
    while True:
        ...
        for i, line in enumerate(lines):
            if i < 2:  # Only display the coordinates of the first two lines
                ...
                if i==0:
                    ...
                    cv2.putText(frame,text1, (200, 30 + i * 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color_text, 1, cv2.LINE_AA)   ### 900 -der   200-izq
                    l_real=hyp
                else:
                    text2=f"WIDTH = {hyp} {W_text}, dist x= {xt}, dist y= {yt}"
                    cv2.putText(frame,text2, (200, 30 + i * 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color_text, 1, cv2.LINE_AA)   ### 900 -der   200-izq
                    w_real=hyp
        ...
        cv2.putText(frame, f"STACK {stack_name}, Length_DTR= {Length_dtr}, Width_DTR= {Width_dtr}", (100, 900), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,0,0), 2, cv2.LINE_AA)  #### 500 -der    100-izq
