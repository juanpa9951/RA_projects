...
def distance_real(col1, col2, row1, row2):
    ...
    start_point_real, idx_low1, idx_high1, tup_low1, tup_high1 = find_closest_tupleV2(tuples_list_pixel,
                                                                                      tuples_list_real,
                                                                                      start_point_pixel)
    end_point_pixel = (col2, row2)
    end_point_real, idx_low2, idx_high2, tup_low2, tup_high2 = find_closest_tupleV2(tuples_list_pixel, tuples_list_real,
                                                                                    end_point_pixel)
...
def guardar_registro_dtr(dtr_register_path, stack_name, Width_dtr, Length_dtr, l_real, w_real, L_text, W_text, batch,
    ...
    # New data to add
    new_data = [
        [stack_name, Length_dtr, l_real, L_text, Width_dtr, w_real, W_text, quality1, quality2, quality3, quality4,
         quality5, quality6, batch, turno, maquina, item_nr, time_now, date]
    ]
...
button_size = (150, 30)
button_rect = (
button_position[0], button_position[1], button_position[0] + button_size[0], button_position[1] + button_size[1])
    ...
    while True:
        ...
        for i, line in enumerate(lines):
            if i < 2:  # Only display the coordinates of the first two lines
                ...
                ### write the text after drawing the 2 lines already......................POSICION TEXTO...............900,200
                if i == 0:
                    text1 = f"LENGTH = {hyp} {L_text}, dist x= {xt}, dist y= {yt}"
                    cv2.putText(frame, text1, (200, 30 + i * 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color_text, 1,
                                cv2.LINE_AA)
                    l_real = hyp
                else:
                    text2 = f"WIDTH = {hyp} {W_text}, dist x= {xt}, dist y= {yt}"
                    cv2.putText(frame, text2, (200, 30 + i * 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color_text, 1,
                                cv2.LINE_AA)
                    w_real = hyp

        # Draw the button
        cv2.rectangle(frame, (button_rect[0], button_rect[1]), (button_rect[2], button_rect[3]), button_color, -1)
        cv2.putText(frame, button_text, (button_rect[0] + 5, button_rect[1] + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (0, 0, 0), 1, cv2.LINE_AA)

        # draw name and mesaures from DTR..................................................POSICION TEXTO........................
        cv2.putText(frame, f"STACK {stack_name}, Length_DTR= {Length_dtr}, Width_DTR= {Width_dtr}", (100, 900),
        ...
        if button_pressed:
            if L_text=="OK" or  L_text=="OK_":     ##### comentar para quitar bloqueo
                if W_text=="OK" or  W_text=="OK_": ##### comentar para quitar bloqueo
...
def submit_action():  #### CAMERA FUNCTION
    ...
    stack_check = (
                DTR.index == stack_name).any()  ### im using stack names as the index of the DTR, here I check if it exists
    if stack_check and quality_check:
        result_message = "Activando Camara..."
        ...
        ######.......... save excel file.................
        guardar_registro_dtr(dtr_register_path, stack_name, Width_dtr, Length_dtr, l_real, w_real, L_text, W_text,
                             batch, turno, maquina, quality1, quality2, quality3, quality4, quality5, quality6, item_nr)
        ######................save log file.......................
        ...
        with open(log_file_path_DTR, "a") as log_file:
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            log_file.write(
                f"{timestamp},{stack_name},{Length_dtr},{l_real},{L_text},{Width_dtr},{w_real},{W_text},{batch}, {turno}, {maquina},{quality1},{quality2},{quality3},{quality4},{quality5},{quality6},{item_nr}\n")
...
def manual_entry(Length_entry, Width_entry, status_label,status_length,status_width):
    try:
        ...
        Width_dtr = DTR.at[stack_name, 'Width']
        Length_dtr = DTR.at[stack_name, 'Length']
        L_text=status_length.cget("text")
        W_text=status_width.cget("text")
        guardar_registro_dtr(dtr_register_path, stack_name, Width_dtr, Length_dtr, l_real, w_real, L_text, W_text, batch,
                             turno, maquina, quality1, quality2, quality3, quality4, quality5, quality6, item_nr)
        with open(log_file_path, "a") as log_file:
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            log_file.write(f"{timestamp},STACK: {stack_name},BATCH: {batch},MAQUINA: {maquina}, ITEM: {item_nr}\n")
...
def verificar_manual(stack_name, Length_entry, Width_entry, status_label, status_length, status_width):
    try:
        ...
        if l_real <= Length_dtr + 30 and l_real >= Length_dtr - 20:
            L_text = "OKm"
            status_length.config(fg="green")
        else:
            L_text = "NOT-OKm"
            status_length.config(fg="red")

        if w_real <= Width_dtr + 30 and w_real >= Width_dtr - 20:
            W_text = "OKm"
            status_width.config(fg="green")
        else:
            W_text = "NOT-OKm"
            status_width.config(fg="red")
...
def manual_button():
    ...
    tk.Button(
        new_window,
        text="Verificar ",
        command=lambda: verificar_manual(stack_name, Length_entry, Width_entry, status_label, status_length,
                                         status_width)
    ).pack(pady=10)
    ...
    tk.Button(
        ...
        command=lambda: manual_entry(Length_entry, Width_entry, status_label,status_length,status_width)
    ).pack(pady=10)
...
label1 = tk.Label(root, text="Work Order:")
