...
def read_and_plot_layerv11(file_path,image_size,Left_centering,Top_centering,Axis_Limit,scale_mode,Reduce_factor,background_color,layer_color,close_image,pattern_mode,raspberry):
    ### error correction using absolute error to define layers, doesn work yet. ITS IMPORTANT TO HAVE LAYER COLORS ASSIGNED IN THE DXF
    import ezdxf
    import pandas as pd
    import matplotlib.pyplot as plt
    from pptx import Presentation
    ...
    for i in range(0,len(x)):    # loop through all the vertices of the splines found
        ...
        if error_x<100 and error_y<100:   # layer detection condition is when there is a closed loop, initial and final points are equal
            check=check+1 # the initial point of the loop
            if check==2: # the final point of the loop
                if pattern_mode==1:
                    Layer_Name_plot = 'PATTERN'
...
file_path='08_COR17_Tagging.dxf'     ####
...
#### V12-NEW VERSION USING LAYER NAMES AS INPUT
read_and_plot_layerv12(file_path,image_size,Left_centering,Top_centering,Axis_Limit,scale_mode,Reduce_factor,background_color,layer_color,close_image,pattern_mode,raspberry)