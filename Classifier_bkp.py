def ClassifierFunction(PathSource,PathDestination,PathLost):
    ...
    Tracker=["p10","p20","p30","p40","p50","p60","p70","p80","p90","p10","p11","p12","p13","p14","p15","p16","p17","p18","p19","ps0","pso"]
    #..................................................
...
def ClassifierFunction_V2(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
...
def ClassifierFunction_v3(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
    ...
    for Name in PhotoNames: # main loop for reading all the photos in the folder
       ...
       picture = picture.save(PathFinal)
       FinalLocations.append(PathFinal)
       print(round((len(FinalLocations)-1)/len(PhotoNames)*100,2),"% Progress, Photo classified to ",PathFinal.replace("Users\Juan Pablo Lopez\PhotoClassification\Destination",""))  # here I print the progress and final destinations
...
def ClassifierFunctionV4(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
...
def ClassifierFunctionV5(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
    ...
    for Name in PhotoNames: # main loop for reading all the photos in the folder
       ...
       response = client.text_detection(image=image)
       texts = response.text_annotations
       ExtractedText = texts[0].description   # aqui està generando el error cuando la fotono tiene ningun texto
       ExtractedText = ExtractedText.lower()
       trip=0 #the trip is for sending photos to Lost folder
       # Here I start the first classification based on PROJECT ####################################
       if "v136" in ExtractedText:
...
def ClassifierFunctionV6(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
    ...
    Nord_viga=["vcp","vcs","vss","vsp"]
    Nord_join=["rj"]
    Nord_acabados=["paca","raca","lrgb","ais","racais","pacais"]
    #tracker for folder name creation
    Tracker=["p10","p20","p30","p40","p50","p60","p70","p80","p90","p11","p12","p13","p14","p15","p16","p17","p18","p19","ps0","pso"]
    #..................................................
    FinalLocations=["Las fotos fueron guardadas en estas ubicaciones"]  #this to initialize the final locations list
...
def ClassifierFunctionV7(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
...
def ClassifierFunctionV8(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
...
def ClassifierFunctionV9(PathSource, PathDestination, PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
    ...
    PreformNordex = ["prs", "prp"]
    Stack = ["scs", "scp"]
    GlassKit = ["pacais", "lrgb", "rj", "cp", "cs", "lbs", "vcs", "vcp", "lp", "ls", "paca", "ais", "raca", "racais",
                "vss", "vsp", "glass"]
    KitsProceso = ["process", "process kit"]
    # sub nomenclatures for glasskit
    Nord_concha = ["-cp", "-cs"]
    ...
    Nord_lrgb = ["-lrgb"]
    # tracker for folder name creation
    Tracker = ["p10", "p20", "p30", "p40", "p50", "p60", "p70", "p80", "p90", "p11", "p12", "p13", "p14", "p15", "p16",
               "p17", "p18", "p19", "ps0", "pso"]
    # ..................................................
    FinalLocations = ["Las fotos fueron guardadas en estas ubicaciones"]  # this to initialize the final locations list
    LostCount = 0
    for Name in PhotoNames:  # main loop for reading all the photos in the folder
        ...
        picture = picture.save(PathFinal)
        FinalLocations.append(PathFinal)
        print(round((len(FinalLocations) - 1) / len(PhotoNames) * 100, 2), "% Progress, Photo classified to ",
              PathFinal.replace("Users\Maria del Carmen\OneDrive - Rewair A S\Inspección - Fotos",
                                ""))  # here I print the progress and final destinations
        Path2Remove = f"{PathSource}\{Name}"  # here I empty the original folder
        os.remove(Path2Remove)
    print("RESULTS: ", len(FinalLocations) - 1 - LostCount, "PHOTOS CLASSIFIED(",
          (len(FinalLocations) - 1 - LostCount) / len(PhotoNames) * 100, "%) and ", LostCount, "PHOTOS LOST")
    return FinalLocations

def ClassifierFunctionV10(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
...
def ClassifierFunctionV11(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
...
def ClassifierFunctionV12(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
...
def ClassifierFunctionV13(PathSource,PathDestination,PathLost):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
    ...
    for Name in PhotoNames: # main loop for reading all the photos in the folder
       ...
       picture = picture.save(PathFinal)
       FinalLocations.append(PathFinal)
       # print(ExtractedText) for debugging
       print(round((len(FinalLocations)-1)/len(PhotoNames)*100,2),"% Progress, Photo classified to ",PathFinal.replace("Users\Maria del Carmen\OneDrive - Rewair A S\Inspección - Fotos",""))  # here I print the progress and final destinations
       Path2Remove=f"{PathSource}\{Name}"  # here I empty the original folder
       os.remove(Path2Remove)
    print("RESULTS: ",len(FinalLocations)-1-LostCount, "PHOTOS CLASSIFIED(",(len(FinalLocations)-1-LostCount)/len(PhotoNames)*100,"%) and ", LostCount, "PHOTOS LOST")
...
def ClassifierFunctionV14(PathSource,PathDestination,PathLost,Path_debug,Mode):
    ...
    print("RESULTS: ",len(FinalLocations)-1-LostCount, "PHOTOS CLASSIFIED(",(len(FinalLocations)-1-LostCount)/len(PhotoNames)*100,"%) and ", LostCount, "PHOTOS LOST")
    return FinalLocations
def ClassifierFunctionV15(PathSource,PathDestination,PathLost,Path_debug,Mode):
    from PIL import Image
    from google.cloud import vision
    import os
    import io
...
def ClassifierFunctionV16(PathSource,PathDestination,PathLost,Path_debug,Mode):
    #####3 THIS VERSION FOR V236 PUTS PALLET FOLDER FIRST AND THEN PUTS THE ODF FOLDER
    from PIL import Image
    from google.cloud import vision
    import os
    import io
...
def ClassifierFunctionV17(PathSource,PathDestination,PathLost,Path_debug,Mode):
    #### THIS VERSION FOR V236 PUTS ODF FOLDER FIRST AND THEN THE PALLETS
    from PIL import Image
    from google.cloud import vision
    import os
    import io
